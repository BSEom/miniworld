...
export default function PhotoPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/who/${userId}`)
      .then(res => res.text())      // json()이 아니라 text()
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const uploadRes = await fetch('/api/album/upload', {
        method: 'POST',
        body: form,
      });
      ...
      const saveRes = await fetch('/api/album/photos', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <h2>📸 {nickname ? `${nickname}의 앨범` : `User ${userId}의 앨범`}</h2>
        ...
        {photos.length === 0 ? (
          ...
          photos.map((p, i) => (
            <div key={i} style={{ border: '1px solid #ddd', padding: 8, borderRadius: 4 }}>
              <h4>{p.title}</h4>
              <img
                src={`/api/album/files/${p.filename}`}
                alt={p.title}
